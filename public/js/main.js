...
let song_num = document.querySelector(".song-num");
song_num.innerHTML = ` ${api.data.length.toString()} Song`;
for (let data of api.data) {
    scrollcontrolls.music_list_container.insertAdjacentHTML("beforeend", `
    ...
    `);
    music_app === null || music_app === void 0 ? void 0 : music_app.insertAdjacentHTML("beforeend", `
...
musicplayercontroll.music_icon.forEach((e, i) => {
...
for (let item of musicplayercontroll.music_cont) {
    item.addEventListener("click", (e) => {
        musicplayercontroll.PlayUi(Array.from(musicplayercontroll.music_cont).indexOf(item));
        Handel_Data();
    });
}
musicplayercontroll.range.addEventListener("click", (e) => {
